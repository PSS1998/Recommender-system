...
id_map = pd.read_csv('the-movies-dataset/links.csv')[['movieId', 'tmdbId']]
...
def hybrid_recommender(title):
    list = []
    for i in range(671):
        list.extend(hybrid(i+1, title)['title'].values.tolist())
    return [k for k, v in sorted(Counter(list).items(), key=lambda item: item[1], reverse=True)][:10]


#print(hybrid_recommender('Taken 3'))
